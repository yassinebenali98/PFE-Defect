...
from dojo.forms import FindingForm, FindingImageFormSet
...
def edit_finding(request, fid):
    ...
    formset = FindingImageFormSet(prefix='finding_images', instance=finding)
    form_error = False
    jform = None
    ...
    if request.method == 'POST':
        form = FindingForm(request.POST, request.FILES, instance=finding, req_resp=None,
                           can_edit_mitigated_data=finding_helper.can_edit_mitigated_data(request.user))
        
        formset = FindingImageFormSet(request.POST, request.FILES, prefix='finding_images', instance=finding)
        if finding.active:
        ...
        if form.is_valid() and formset.is_valid() and (jform is None or jform.is_valid()):
            if jform:
