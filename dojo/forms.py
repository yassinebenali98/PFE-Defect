...
from .models import Finding, FindingImage
from django.utils.safestring import mark_safe
...
class ProductForm(forms.ModelForm):
    ...
    logo = forms.ImageField(widget=forms.widgets.FileInput,
        label="logo",
    ...
    class Meta:
        model = Product
        fields = ['name','logo', 'description', 'tags', 'product_manager', 'technical_contact', 'team_manager', 'prod_type', 'sla_configuration', 'regulations',
...
class FindingImageForm(ModelForm):
    class Meta:
        model = FindingImage
        fields = ('title', 'image',)

FindingImageFormSet = inlineformset_factory(
    Finding,
    FindingImage,
    form=FindingImageForm,
...
class AddFindingForm(forms.ModelForm):
    ...
    PRIORITE_CHOICES = [
        ...
        ('Moyenne Terme', 'Moyenne Terme'),
        ('Long Terme', 'Long Terme'),
        ]
    STATUT_CHOICES = [
        ('Résolue', 'Résolue'),
        ('Non Résolue', 'Non Résolue'),
        ]
    type = forms.ChoiceField(choices=TYPE_CHOICES, widget=forms.Select())
    complexite=forms.ChoiceField(choices=COMPLEXITE_CHOICES, widget=forms.Select())
    priorite=forms.ChoiceField(choices=PRIORITE_CHOICES, widget=forms.Select())
    statut=forms.ChoiceField(choices=STATUT_CHOICES, widget=forms.Select())
...
class AdHocFindingForm(forms.ModelForm):
    ...
    # the only reliable way without hacking internal fields to get predicatble ordering is to make it explicit
    field_order = ('title','vulnerability_ids','statut','severity', 'cvssv3', 'endpoints', 'endpoints_to_add', 'description','steps_to_reproduce', 'mitigation','type','complexite','priorite', 'impact','cwe','date', 'request', 'response', 
                   'severity_justification',  'references', 'active', 'verified', 'false_p', 'duplicate', 'out_of_scope',
