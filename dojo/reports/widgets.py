...
class AvantProposForm(forms.Form):
    titre = forms.CharField(max_length=200, required=False, help_text="The main report heading.")
...
class AvantPropos(Widget):
    def __init__(self, *args, **kwargs):
        super(AvantPropos, self).__init__(*args, **kwargs)
        self.title = 'Avant Propos'
        self.form = AvantProposForm()
        self.help_text = "Avant propos"

    def get_html(self):
        return render_to_string("dojo/custom_html_report_avant_propos.html", {"titre": self.title})
...
def report_widget_factory(json_data=None, request=None, user=None, finding_notes=False, finding_images=False,
    ...
    for idx, widget in enumerate(widgets):
        ...
        if list(widget.keys())[0] == 'avant-propos':
            avant_propos = AvantPropos(request=request)
            avant_propos.title = next((item for item in widget.get(list(widget.keys())[0]) if item["name"] == 'titre'), None)[
                'value']
            
            selected_widgets[list(widget.keys())[0]] = avant_propos
