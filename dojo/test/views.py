...
from dojo.forms import NoteForm, TestForm,FindingImageForm, \
...
from dojo.models import IMPORT_UNTOUCHED_FINDING, Finding, Finding_Group, Test, Note_Type, BurpRawRequestResponse, Endpoint, Stub_Finding, \
    Finding_Template, Cred_Mapping, Test_Import, Product_API_Scan_Configuration, Test_Import_Finding_Action,FindingImage
...
def add_findings(request, tid):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and (jform is None or jform.is_valid()) and image_formset.is_valid():
        ...
        image_formset = FindingImageFormSet(queryset=FindingImage.objects.none())
        if use_jira:
